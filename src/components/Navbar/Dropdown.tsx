        ...
        >
          <div className="rounded flex flex-col bg-white text-white w-64 p-1 shadow">
            {this.props.dropDownItems.map(item => (
                <Link onClick={()=> this.setState({isActive: false})} to={'/'+item.split(' ').join('')} key={item} style={{ textDecoration: 'none' }} className="w-full transition-none rounded hover:bg-gray-200 text-gray-600  hover:text-gray-800 text-lg text-left px-2 py-1"> {item} </Link>
            ))}
